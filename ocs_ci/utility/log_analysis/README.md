...
3. **Classifies** remaining failures using AI (Claude) into categories: `product_bug`, `test_bug`, `infra_issue`, `flaky_test`, or `unknown`
...
| `-f`, `--format` | `markdown` | Output format: `json`, `markdown` |
| `-o`, `--output` | stdout | Write report to file |
| `-v`, `--verbose` | off | Enable debug logging |

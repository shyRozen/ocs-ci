...
class FailureClassifier:
    ...
    def classify_failures(
        ...
        for cache_key, group in signature_groups.items():
            ...
            if self.ai_backend.requires_budget_limit and ai_call_count >= self.max_failures:
                logger.warning(
            ...
            try:
                ...
                logger.warning(f"AI classification failed for {representative.name}: {e}")
                analysis_dict = {
                    "category": "unknown",
                    "confidence": 0.0,
    ...
    @staticmethod
    def _build_analysis(test_result: TestResult, analysis_dict: dict) -> FailureAnalysis:

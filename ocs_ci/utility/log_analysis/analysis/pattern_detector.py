...
from ocs_ci.utility.log_analysis.analysis.history_store import RunRecord
...
class PatternDetector:
    ...
    def detect_flaky_tests(
        ...
        for test_name, entries in self._test_timeline.items():
            ...
            if total_decisive == 0 or pass_count == 0 or (fail_count + error_count) == 0:
                continue
    ...
    def detect_regressions(self, min_consecutive_failures: int = 2) -> list:
        ...
        for test_name, entries in self._test_timeline.items():
            ...
            previous_passes = sum(
                1 for e in entries_before if e["status"] == "passed"
            )
            ...
            if failure_start_idx > 0:
                ...
                version_changed = prev_version != fail_version and prev_version and fail_version

            # Most common failure category
            fail_categories = [
                e["category"]
                for e in entries[failure_start_idx:]
                if e.get("category")
            ]
            category = Counter(fail_categories).most_common(1)[0][0] if fail_categories else ""
    ...
    def build_trend_report(self) -> TrendReport:
        ...
        # Squad health
        squad_stats = defaultdict(lambda: {"total_tests": 0, "pass_count": 0, "fail_count": 0, "flaky_count": 0})
        for test_name, entries in self._test_timeline.items():
        ...
        for squad in squad_stats:
            ...
            stats["pass_rate"] = round(stats["pass_count"] / total, 3) if total > 0 else 0
    ...
    def annotate_run(self, run_analysis) -> None:
        ...
        flaky_map = {
            t.test_name: t for t in self.detect_flaky_tests(min_runs=2)
        }
        regression_map = {
            r.test_name: r for r in self.detect_regressions()
        }
